...
def upgrade() -> None:
    ...
    type_exists = connection.execute(text("SELECT 1 FROM pg_type WHERE typname = 'role_name'")).fetchone()
    ...
    op.create_table(
        ...
        sa.Column(
            "name",
            postgresql.ENUM("STUDENT", "TEACHER", "ADMIN", name="role_name", create_type=False),
            nullable=False,
        ),
